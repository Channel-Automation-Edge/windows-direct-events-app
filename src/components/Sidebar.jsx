...
const Sidebar = () => {
  ...
  useEffect(() => {
    // Get brand settings from localStorage since they're handled globally now
    ...
    <aside className="
      ...
      <div className="mb-6 hidden sm:flex sm:justify-center">
        <img 
          ...
          alt={`${brandSettings?.name || 'Company'} Logo`} 
          className="h-8 w-auto max-w-[270px]" 
        />
      </div>
      ...
      <nav className="flex sm:flex-col items-center justify-around w-full sm:space-y-8">
        <NavLink 
          to={getNavigationUrl("/")} 
          className={({isActive}) => `flex flex-row items-center justify-center gap-2 px-3 py-2 rounded-full transition-colors duration-200 active:bg-transparent ${
        ...
        <NavLink 
          to={getNavigationUrl("/new-appointment")} 
          className={({isActive}) => `flex flex-row items-center justify-center gap-2 px-3 py-2 rounded-full transition-colors duration-200  active:bg-transparent ${
            isActive ? 'text-brand' : 'text-gray-600 hover:text-brand hover:bg-gray-50'
          } sm:flex-col sm:gap-0 sm:rounded-md sm:px-2`}
        >
